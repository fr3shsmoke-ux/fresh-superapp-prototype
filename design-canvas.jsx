...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useEffect(() => {
    ...
    const onWheel = (e) => {
      // If wheel is over a scrollable element inside an artboard, let the
      // browser scroll it instead of zooming the canvas. This is what makes
      // the iPhone-frame content scroll naturally with the mouse wheel.
      let el = e.target;
      while (el && el !== vp && el.nodeType === 1) {
        if (el.hasAttribute && el.hasAttribute('data-dc-slot')) break;
        const cs = getComputedStyle(el);
        const oy = cs.overflowY;
        if ((oy === 'auto' || oy === 'scroll') && el.scrollHeight > el.clientHeight + 1) {
          return; // do not preventDefault — element scrolls itself
        }
        el = el.parentElement;
      }
      e.preventDefault();
